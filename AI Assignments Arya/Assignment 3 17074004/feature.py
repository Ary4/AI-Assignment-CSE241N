...
class BagOfWordsFeatureExtractor(object):
    ...
    def preprocess(self, documents):
        ...
        for w in list(remove_words):
            word_freq.pop(w)
        c=0
        for w in list(word_freq):
            self.idx_to_word[c] = w
            self.word_to_idx[w] = c
            c+=1
        ...
        for idx, doc in enumerate(documents):
            ...
            for w in words:
                ...
                if w in self.word_to_idx:
                    features[idx][self.word_to_idx[w]] += 1/len(words)
...
class TfIdfFeatureExtractor(object):
    ...
    def preprocess(self, documents):
        ...
        for doc in documents:
            ...
            for w in set(words):
                ''' YOUR CODE HERE '''
                if w not in doc_word_freq.keys():
                    doc_word_freq[w] = 1
                else:
                    doc_word_freq[w] += 1
        ...
        # 'doc_word_freq'.
        for w in list(remove_words):
            total_word_freq.pop(w)
            doc_word_freq.pop(w)
        
        word_freq_tensor = np.zeros(len(doc_word_freq))
        c=0
        for w in list(doc_word_freq):
            self.idx_to_word[c] = w
            self.word_to_idx[w] = c
            word_freq_tensor[c] = doc_word_freq[w]
            c+=1
