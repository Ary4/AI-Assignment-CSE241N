...
def __applyResolution(resList, idx, quiet=False):
    ...
    newResList = []
    newResList.extend(resList)
    index=0
    flag = False
    for index in range(len(resList)):
        for atomic in resList[index]:
            for k in range(index+1, len(resList)):
                if flag is False and Not(atomic).toCNF() in resList[k]:  #and k != index
                    l = []
                    l=resList[index].copy()
                    l.remove(atomic)
                    l+=resList[k]
                    l.remove(Not(atomic).toCNF())
                    if l not in newResList:
                        newResList.append(l)
                        flag = True
                        break
                    else:
...
def resolutionRefutation(axioms, proposition, quiet=False):
    ...
    while True:
        ...
        newRes = __applyResolution(oldRes, idx)
