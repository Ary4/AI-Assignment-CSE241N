...
class Atom(Formula):
    ...
    def truthValue(self, truthDict):
        ...
        if self.name in truthDict.keys():
...
class Or(BinaryFormula):
    ...
    def toCNF(self):
        ...
        while currIndex < len(flattenList1):
            if isinstance(flattenList1[currIndex], And):
                flattenList1 += [flattenList1[currIndex].arg1, flattenList1[currIndex].arg2]
                flattenList1.pop(currIndex)
            else:
                currIndex += 1
        ...
        while currIndex < len(flattenList2):
            if isinstance(flattenList2[currIndex], And):
                flattenList2 += [flattenList2[currIndex].arg1, flattenList2[currIndex].arg2]
                flattenList2.pop(currIndex)
            else:
                currIndex += 1

        final = None
        flag = False
        for pi in flattenList1:
            for qi in flattenList2:
                if flag==False:
                    final = Or(pi, qi)
                    flag = True
                else:
                    final = And(final, Or(pi,qi))
        return final
