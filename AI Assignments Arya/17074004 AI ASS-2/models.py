...
class linear_model:
    ...
    def backward(self, y_actl, y_pred, feat):
        ...
        differen = y_pred - y_actl
        slope = np.matmul(feat.transpose(), differen) / (y_pred.shape[0])
        self.W = self.W - self.lr * slope
...
def calc_features(X, choice, param=5):
    ...
    for x in X:
        if choice == 'linear':
            ...
            a = X.shape[0]
            b = X.shape[1] + 1             #features + bias column
            feat = np.ones((a, b))
            feat[:,:-1] = X
            return feat
            ### YOUR CODE SHOULD END HERE ###
